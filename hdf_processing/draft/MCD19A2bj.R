...
library(lubridate)
library(sp)
setwd("D:/Auguste/UChicago/2018-2019/2019winter/GIS2/project/tif1")
...
bj2017jan <- stack(aod2017[1:31])
bj_2017jan.mean <- mean(bj2017jan, na.rm=TRUE)

bj2017feb <- stack(aod2017[32:58])
bj_2017feb.mean <- mean(bj2017feb, na.rm=TRUE)

bj2017mar <- stack(aod2017[59:90])
bj_2017mar.mean <- mean(bj2017mar, na.rm=TRUE)

bj2017apr <- stack(aod2017[91:121])
bj_2017apr.mean <- mean(bj2017apr, na.rm=TRUE)

bj2017may <- stack(aod2017[122:153])
bj_2017may.mean <- mean(bj2017may, na.rm=TRUE)

bj2017jun <- stack(aod2017[154:184])
bj_2017jun.mean <- mean(bj2017jun, na.rm=TRUE)

bj2017jul <- stack(aod2017[185:216])
bj_2017jul.mean <- mean(bj2017jul, na.rm=TRUE)

bj2017aug <- stack(aod2017[217:248])
bj_2017aug.mean <- mean(bj2017aug, na.rm=TRUE)

bj2017sep <- stack(aod2017[249:279])
bj_2017sep.mean <- mean(bj2017sep, na.rm=TRUE)

bj2017oct <- stack(aod2017[280:311])
bj_2017oct.mean <- mean(bj2017oct, na.rm=TRUE)

bj2017nov <- stack(aod2017[312:342])
bj_2017nov.mean <- mean(bj2017nov, na.rm=TRUE)

bj2017dec <- stack(aod2017[343:363])
bj_2017dec.mean <- mean(bj2017dec, na.rm=TRUE)

bj2017monthly.avg <- stack(bj_2017sep.mean, bj_2017oct.mean, bj_2017nov.mean, bj_2017may.mean, bj_2017mar.mean, bj_2017jun.mean, bj_2017jul.mean, bj_2017jan.mean, bj_2017feb.mean, bj_2017dec.mean, bj_2017aug.mean, bj_2017apr.mean)
bj2017monthly.avg.mean <- mean(bj2017monthly.avg, na.rm=TRUE)
plot(bj2017monthly.avg.mean)
...
aod.stack <- raster::stack(all.aod)
